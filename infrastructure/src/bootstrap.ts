...
import { accounts, Dev, Staging, Prod } from './pipeline';


async function main() {
  // make sure all accounts are setup
  for (const accountName of ['toolchain', 'dev', 'staging', 'production'] as const) {
    const account = accounts[accountName];
    accounts[accountName] = await promptAccount(`Select AWS profile for '${accountName}' account`, account?.profile);
  }
  Dev.account = accounts.dev;
  Staging.account = accounts.staging;
  ...
  for (let account of [Dev, Staging, Prod]) {
